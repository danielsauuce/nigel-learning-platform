      ...
      <div className="flex-1 lg:ml-80 p-6 md:p-12 overflow-y-auto">
        <div className="max-w-5xl mx-auto">
          <header className="flex justify-between items-center mb-12">
            ...
            <button className="bg-[#22223B] text-white font-bold px-8 py-3 rounded-2xl flex items-center gap-2 shadow-lg">
          ...
          <div className="grid lg:grid-cols-4 gap-12">
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={toggleTheme}
                className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-bold transition-all ${dark ? 'text-gray-400 hover:text-white' : 'text-gray-400 hover:text-[#22223B]'}`}
              >
                {dark ? (
            ...
            <div className="lg:col-span-3 space-y-8">
              {activeTab === 'profile' && (
                <>
                  <div
                    className={`p-10 rounded-[3rem] border ${dark ? 'bg-[#2A2A40] border-[#3A3A55]' : 'bg-white border-gray-100'}`}
                  >
                      ...
                      {[
                        ...
                        <div key={field.label}>
                          ...
                          <input
                            ...
                            className={`w-full p-4 rounded-2xl border font-medium ${dark ? 'bg-[#1A1A2E] border-[#3A3A55] text-white' : 'bg-gray-50 border-gray-200 text-[#22223B]'}`}
                          />
                        </div>
                      ))}
                    </div>
                  ...
                  <div
                    className={`p-8 rounded-[3rem] border ${dark ? 'bg-[#2A2A40] border-[#B9A7F8]/15' : 'bg-white border-[#B9A7F8]/15'}`}
                  >
                    ...
                    >
                      Invite parents to track student progress and celebrate
                      milestones together.
                    </p>

                    <button
                      onClick={() => setFamilyConsent(!familyConsent)}
                      className={`w-full flex items-start gap-3 p-4 rounded-2xl border mb-4 text-left ${dark ? 'bg-[#1A1A2E] border-[#3A3A55]' : 'bg-gray-50 border-gray-100'}`}
                    >
                      <div
                        className={`w-5 h-5 rounded-md border-2 flex items-center justify-center mt-0.5 ${familyConsent ? 'border-[#B9A7F8] bg-[#B9A7F8]' : dark ? 'border-[#3A3A55]' : 'border-gray-200'}`}
                      >
                        {familyConsent && (
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[10px] text-gray-400 leading-tight">
                          By checking this, you allow invited family members to
                          see daily activity, XP, and lesson completion status.
                        </p>
                      </div>
                    </button>
                    ...
                    {familyConsent && (
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-2">
                          <div
                            className={`flex-1 flex items-center gap-2 px-4 py-3 rounded-xl border ${dark ? 'bg-[#B9A7F8]/5 border-[#B9A7F8]/15' : 'bg-[#B9A7F8]/5 border-[#B9A7F8]/15'}`}
                          >
                          ...
                          <button
                            className={`w-10 h-10 rounded-xl border flex items-center justify-center ${dark ? 'border-[#3A3A55]' : 'border-gray-200'}`}
                          >
                            <Copy className="w-4 h-4 text-gray-400" />
                          </button>
                        </div>
                        <button className="w-full flex items-center justify-center gap-2 bg-[#B9A7F8] text-white font-bold py-3 rounded-xl">
              ...
              {activeTab === 'notifications' && (
                ...
                >
                  ...
                  {[
                    ...
                    <div
                      key={n.label}
                      className={`flex items-center justify-between py-5 border-b last:border-b-0 ${dark ? 'border-[#3A3A55]' : 'border-gray-100'}`}
                    >
                      <div>
                        <p
                      ...
                      <div className="w-12 h-7 bg-[#B9A7F8] rounded-full relative">
                        <div className="w-5 h-5 bg-white rounded-full absolute right-1 top-1 shadow" />
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {activeTab === 'security' && (
                ...
                >
                  ...
                  <div className="space-y-6">
                    ...
                    <button className="bg-[#22223B] text-white font-bold px-8 py-4 rounded-2xl">
                      Update Password
                    </button>
                  </div>
                </div>
