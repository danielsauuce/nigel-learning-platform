        ...
        <div className="max-w-6xl mx-auto">
          <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              <button className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm text-gray-400">
                ...
                <tbody className="divide-y divide-gray-100">
                  {STUDENTS.map((student, i) => (
                    ...
                    >
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-2 bg-gray-100 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full ${student.avg > 85 ? 'bg-emerald-400' : student.avg > 70 ? 'bg-amber-400' : 'bg-rose-400'}`}
                              style={{ width: `${student.avg}%` }}
                            />
                          </div>
                          <span className="text-xs font-bold text-[#22223B]">
                            {student.avg}%
                          </span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <span
                          className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold ${
                            student.simLevel === 'HIGH'
                              ? 'bg-emerald-50 text-emerald-600'
                              : student.simLevel === 'STEADY'
                      ...
                      <td className="px-8 py-6">
                        <span
                          className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold ${student.active ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}
                        >
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${student.active ? 'bg-green-600' : 'bg-red-600'}`}
                          />
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button className="p-2 bg-gray-100 rounded-lg text-gray-500 hover:bg-[#B9A7F8] hover:text-white transition-all">
                            <Mail className="w-4 h-4" />
                          </button>
                          <button className="p-2 bg-gray-100 rounded-lg text-gray-500 hover:bg-[#22223B] hover:text-white transition-all">
