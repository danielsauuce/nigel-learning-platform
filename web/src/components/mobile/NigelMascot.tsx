...
export const NigelMascot = ({ size = 160, animate = true }: { size?: number; animate?: boolean }) => {
  const w = size
  const h = size * 1.35
  const Inner = (
...
export const NigelMascotWaving = ({ size = 64 }: { size?: number }) => {
  ...
  return (
    <svg width={w} height={h} viewBox="0 0 200 280" fill="none">
      ...
      <ellipse cx="155" cy="115" rx="16" ry="12" fill="#F7B6B6" opacity={0.75} />
      {/* Left arm down */}
      <path d="M18 140 Q8 175 18 210 Q22 220 26 210 Q32 180 28 145 Z" fill="white" opacity={0.85} />
      {/* Right arm waving up */}
      <path d="M185 90 Q210 50 195 15 Q190 8 185 15 Q175 45 180 85 Z" fill="white" opacity={0.85} />
      {/* Wave marks */}
      <path d="M198 25 Q205 20 198 15" stroke="#F7E6B6" strokeWidth="2" strokeLinecap="round" fill="none" />
      <path d="M205 30 Q212 25 205 20" stroke="#F9D6D0" strokeWidth="2" strokeLinecap="round" fill="none" />
    </svg>
  )
}

/** Small puzzle icon for logos */
export const NigelPuzzleIcon = ({ size = 20, color = 'white' }: { size?: number; color?: string }) => (
