  ...
  <div className="w-[280px] h-[600px] bg-white rounded-[3rem] p-5 flex flex-col gap-3 shadow-2xl overflow-hidden">
    ...
    <div className="flex items-center justify-between px-2 pt-1">
      ...
      <div className="flex items-center gap-1.5">
        <svg width="12" height="10" viewBox="0 0 16 12" fill="none"><path d="M1 8h2v4H1zM5 5h2v7H5zM9 2h2v10H9zM13 0h2v12h-2z" fill="#22223B" opacity=".5"/></svg>
      </div>
    </div>
    ...
    <div className="bg-edulite-bg rounded-2xl p-4 flex items-center justify-between">
      <div className="flex-1">
        <h3 className="text-xs font-bold text-edulite-navy leading-tight">
          Budget lesson<br />in 30 minutes
        </h3>
    ...
    <div>
      <div className="flex justify-between items-center mb-1.5">
        ...
        <span className="text-[8px] text-edulite-purple font-medium">See more</span>
      </div>
    ...
    <div className="flex gap-2">
      {[
        ...
        <div key={stat.label} className={`flex-1 ${stat.bg} rounded-xl py-2 text-center`}>
          <span className="text-[10px] block">{stat.emoji}</span>
          <span className="text-[10px] font-bold text-edulite-navy block">{stat.value}</span>
    ...
    <div className="bg-edulite-navy rounded-2xl p-3">
      <div className="flex items-center gap-1 mb-1">
        ...
        <span className="text-[7px] font-bold text-edulite-yellow tracking-wider uppercase">Daily Challenge</span>
      </div>
      <p className="text-[9px] font-bold text-white mb-0.5">Budget Builder Challenge</p>
      <p className="text-[7px] text-white/50 mb-2">Allocate income using the 50/30/20 rule!</p>
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          <span className="text-[7px] text-white/40 flex items-center gap-0.5">⏱ 5 Mins</span>
          <span className="text-[7px] text-white/40 flex items-center gap-0.5">✨ 50 XP</span>
        </div>
    ...
    <div>
      <div className="flex justify-between items-center mb-1">
        <span className="text-[9px] font-bold text-edulite-navy">Achievements</span>
        <span className="text-[7px] text-edulite-purple font-medium">See more</span>
      </div>
      <div className="flex gap-1.5">
        {[
          ...
          <div key={b.label} className="flex-1 bg-gray-50 rounded-xl py-1.5 text-center">
            <span className="text-xs block">{b.emoji}</span>
            <span className="text-[6px] text-gray-400 block mt-0.5">{b.label}</span>
          </div>
        ))}
      </div>
    </div>
