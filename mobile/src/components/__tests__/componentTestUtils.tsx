...
export const createComponentProps = (filePath: string) => {
  const componentFilePath = filePath.endsWith('.test.tsx')
    ? filePath.replace(/\.test\.tsx$/, '.tsx')
    : filePath;
  const relativePath = path.relative(process.cwd(), componentFilePath);
...
export const renderComponent = <TProps extends object>(
  filePath: string,
  Component: React.ComponentType<TProps>,
) => {
  const props = createComponentProps(filePath) as TProps;
  let result: TestRenderer.ReactTestRenderer | undefined;

  act(() => {
    result = TestRenderer.create(<Component {...props} />);
  });
