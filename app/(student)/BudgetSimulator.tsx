...
const BUDGET_CATEGORIES: BudgetCategory[] = [
  { id: "rent", label: "Rent / Board", emoji: "🏠", color: "#4FC3F7", suggestedPercent: 30, min: 0, max: 60 },
  { id: "travel", label: "Travel", emoji: "🚌", color: "#A855F7", suggestedPercent: 10, min: 0, max: 30 },
  { id: "food", label: "Food & Groceries", emoji: "🍕", color: "#10B981", suggestedPercent: 15, min: 0, max: 40 },
  { id: "phone", label: "Phone Bill", emoji: "📱", color: "#F59E0B", suggestedPercent: 5, min: 0, max: 15 },
  { id: "subscriptions", label: "Subscriptions", emoji: "📺", color: "#FF2E91", suggestedPercent: 5, min: 0, max: 20 },
  { id: "savings", label: "Savings", emoji: "🐷", color: "#FFD700", suggestedPercent: 15, min: 0, max: 50 },
  { id: "fun", label: "Fun Spending", emoji: "🎮", color: "#3B82F6", suggestedPercent: 20, min: 0, max: 50 },
];
...
// ── Floating Coin ──
const FloatingCoin = ({
  delay, startX, startY, size, opacity,
}: {
  delay: number; startX: number; startY: number; size: number; opacity: number;
}) => {
...
const BottomWave = () => {
  ...
  return (
    <Animated.View style={[styles.waveContainer, { transform: [{ translateX: waveTranslateX }] }]} pointerEvents="none">
    ...
    {STEP_LABELS.map((label, i) => {
      ...
      return (
        <View key={label} style={styles.stepItem}>
          ...
          <Text style={[styles.stepLabel, (isActive || isDone) && { color: "rgba(255,255,255,0.7)" }]}>{label}</Text>
...
// ── Formatting ──
const formatPound = (n: number) => `£${n.toLocaleString("en-GB", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`;

// ── Mini Progress Bar ──
const MiniProgressBar = ({ progress, color }: { progress: number; color: string }) => (
  <View style={styles.miniProgressTrack}>
    <View style={[styles.miniProgressFill, { width: `${Math.min(progress, 1) * 100}%`, backgroundColor: color }]} />
  </View>
);
  ...
  const play = useCallback(() => {
    ...
    const anims = fades.map((fade, i) =>
      Animated.parallel([
        ...
        Animated.timing(slides[i], { toValue: 0, duration: 380, delay: i * 80, useNativeDriver: true }),
      ])
    );
    ...
    <>
      ...
      <Animated.View style={anim(1)}>
        <Text style={styles.stepDescription}>
          Imagine you've just finished school and landed your first job. Each role has a different salary — tap one to get started!
        </Text>
      </Animated.View>
            ...
            >
              ...
              <View style={styles.jobCardText}>
                ...
                <View style={styles.jobSalaryRow}>
                  <Text style={[styles.jobSalary, { color: job.color }]}>{formatPound(job.grossAnnual)}/year</Text>
                </View>
              </View>
...
const PayslipStep = ({
  ...
  return (
    <>
      <Animated.View style={anim(0)}>
        <View style={styles.stepHeaderRow}>
          <View style={[styles.stepIconCircle, { backgroundColor: `${job.color}18`, borderColor: `${job.color}20` }]}>
      ...
      <Animated.View style={anim(1)}>
        <Text style={styles.stepDescription}>
          Before your money hits your bank, some is taken for tax and National Insurance. Here's what happens to your pay each month.
        </Text>
      </Animated.View>
      ...
      <Animated.View style={anim(2)}>
        <View style={styles.payslipCard}>
          ...
          >
            ...
            <Text style={styles.payslipHeaderRole}>{job.emoji} {job.title}</Text>
          </LinearGradient>
          ...
          <View style={styles.payslipRow}>
            ...
            <Text style={[styles.payslipAmountGross, { color: "#10B981" }]}>{formatPound(job.takeHome)}</Text>
          </View>
        </View>
      </Animated.View>
      ...
      <Animated.View style={anim(3)}>
        <View style={styles.payslipBarContainer}>
          <View style={styles.payslipBarTrack}>
            <View style={[styles.payslipBarFill, { width: `${100 - deductionPercent}%`, backgroundColor: "#10B981" }]} />
            <View style={[styles.payslipBarFill, { width: `${deductionPercent}%`, backgroundColor: "#EF4444", opacity: 0.7 }]} />
          </View>
      ...
      <Animated.View style={anim(4)}>
        <View style={styles.infoBox}>
          ...
          <Text style={styles.infoBoxText}>
            In the UK, you don't pay any income tax on the first £12,570 you earn each year. This is called your "Personal Allowance".
          </Text>
        </View>
      </Animated.View>
      ...
      <Animated.View style={anim(5)}>
        <TouchableOpacity activeOpacity={0.85} onPress={onContinue} style={{ marginTop: 8 }}>
          <LinearGradient colors={["#FFD700", "#F5A623"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.ctaButton}>
  ...
  return (
    <View style={styles.sliderContainer}>
      ...
      <View style={styles.sliderTrackRow}>
        <TouchableOpacity onPress={decrement} style={[styles.sliderBtn, { borderColor: `${category.color}40` }]} activeOpacity={0.6}>
          <Text style={[styles.sliderBtnText, { color: category.color }]}>−</Text>
        </TouchableOpacity>

        <View style={styles.sliderTrack}>
          <View style={[styles.sliderFill, { width: `${percent}%`, backgroundColor: category.color }]} />
          {/* Suggested marker */}
          <View style={[styles.sliderSuggested, { left: `${category.suggestedPercent}%` }]}>
            <View style={[styles.sliderSuggestedLine, { backgroundColor: `${category.color}60` }]} />
          </View>
        </View>

        <TouchableOpacity onPress={increment} style={[styles.sliderBtn, { borderColor: `${category.color}40` }]} activeOpacity={0.6}>
          <Text style={[styles.sliderBtnText, { color: category.color }]}>+</Text>
        </TouchableOpacity>
      </View>

      <Text style={styles.sliderSuggestedText}>
        Suggested: ~{category.suggestedPercent}% ({formatPound(Math.round(maxBudget * category.suggestedPercent / 100))})
      </Text>
    </View>
  );
};
      ...
      <View style={styles.moneyLeftTop}>
        <View>
          ...
          <Text style={[styles.moneyLeftAmount, { color: barColor }]}>
            {isOver ? "−" : ""}{formatPound(Math.abs(remaining))}
          </Text>
        </View>
        <View>
          <Text style={styles.moneyLeftLabel}>Total Budget</Text>
          <Text style={styles.moneyLeftSpent}>{formatPound(spent)} / {formatPound(takeHome)}</Text>
        </View>
      </View>
      <View style={styles.moneyLeftTrack}>
        <View style={[styles.moneyLeftFill, { width: `${Math.min(percent, 100)}%`, backgroundColor: barColor }]} />
      </View>
      {isOver && (
...
const BudgetStep = ({
  ...
  return (
    <>
      <Animated.View style={anim(0)}>
        <View style={styles.stepHeaderRow}>
          <View style={[styles.stepIconCircle, { backgroundColor: "rgba(16,185,129,0.15)", borderColor: "rgba(16,185,129,0.2)" }]}>
            <Text style={{ fontSize: 28 }}>💰</Text>
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.stepTitle}>Build Your Budget</Text>
            <Text style={styles.stepSubtitle}>Allocate your {formatPound(job.takeHome)} take-home pay</Text>
          </View>
        </View>
      </Animated.View>
      ...
      <Animated.View style={anim(1)}>
        <Text style={styles.stepDescription}>
          Use the + and − buttons to decide how much to spend in each area. Try to keep within your take-home pay!
        </Text>
      </Animated.View>
      ...
      <Animated.View style={anim(BUDGET_CATEGORIES.length + 3)}>
        ...
        >
          <LinearGradient
            colors={isValid ? ["#FFD700", "#F5A623"] : ["rgba(255,215,0,0.25)", "rgba(245,166,35,0.15)"]}
...
const LifeEventStep = ({
  ...
  return (
    <>
      <Animated.View style={anim(0)}>
        <View style={styles.stepHeaderRow}>
          <View style={[styles.stepIconCircle, { backgroundColor: "rgba(239,68,68,0.15)", borderColor: "rgba(239,68,68,0.2)" }]}>
            <Text style={{ fontSize: 28 }}>🎲</Text>
          </View>
          <View style={{ flex: 1 }}>
      ...
      <Animated.View style={[anim(1), { transform: [{ translateX: shakeTranslate }] }]}>
        ...
        >
          ...
          <View style={[styles.lifeEventCostPill, { backgroundColor: `${event.color}20` }]}>
            <Text style={[styles.lifeEventCostText, { color: event.color }]}>Cost: {formatPound(event.cost)}</Text>
          </View>
        </LinearGradient>
      </Animated.View>
      ...
      <Animated.View style={anim(2)}>
        <View style={styles.impactCard}>
          ...
          <View style={styles.impactRow}>
            ...
            <Text style={[styles.impactValue, { color: "#EF4444" }]}>−{formatPound(event.cost)}</Text>
          </View>
          <View style={styles.payslipDivider} />
          <View style={styles.impactRow}>
          ...
          {canAfford ? (
            <View style={[styles.impactBanner, { backgroundColor: "rgba(16,185,129,0.1)", borderColor: "rgba(16,185,129,0.2)" }]}>
            ...
            <View style={[styles.impactBanner, { backgroundColor: "rgba(239,68,68,0.1)", borderColor: "rgba(239,68,68,0.2)" }]}>
              <Text style={styles.impactBannerEmoji}>😬</Text>
              <Text style={[styles.impactBannerText, { color: "#EF4444" }]}>
                Your savings didn't cover the full cost. You'd need to borrow or cut spending elsewhere.
              </Text>
            </View>
          )}
        </View>
...
const ResultsStep = ({
  ...
  // Feedback tips
  const tips: string[] = [];
  if (savingsPercent < 10) tips.push("Try setting aside at least 10% of your income as savings — even small amounts add up.");
  if ((budgetValues["subscriptions"] ?? 0) > job.takeHome * 0.1) tips.push("Your subscriptions are quite high. Could you share a family plan or drop one?");
  if ((budgetValues["fun"] ?? 0) > job.takeHome * 0.25) tips.push("Fun spending is important, but keeping it under 25% gives you more breathing room.");
  if (couldAffordEvent) tips.push("Great work — you had enough savings to handle an unexpected cost!");
  ...
  return (
    <>
      {/* Confetti */}
      {stars >= 4 && confettiAnims.map((ca, i) => {
        const angle = (i / 10) * Math.PI * 2;
        const dist = 70 + (i % 3) * 25;
        const translateX = ca.interpolate({ inputRange: [0, 1], outputRange: [0, Math.cos(angle) * dist] });
        const translateY = ca.interpolate({ inputRange: [0, 1], outputRange: [0, Math.sin(angle) * dist - 30] });
        const opacity = ca.interpolate({ inputRange: [0, 0.7, 1], outputRange: [1, 1, 0] });
        const colors = ["#FFD700", "#FF2E91", "#4FC3F7", "#10B981", "#A855F7", "#F59E0B", "#FF6B6B", "#70E0A0", "#FFD700", "#FF2E91"];
        return (
          <Animated.View key={`rc-${i}`} pointerEvents="none"
            style={{ position: "absolute", left: width / 2 - 4, top: 180, width: 8, height: 8, borderRadius: i % 2 === 0 ? 4 : 1, backgroundColor: colors[i], opacity, transform: [{ translateX }, { translateY }], zIndex: 20 }}
          />
        );
      })}
      ...
      <Animated.View style={anim(0)}>
        <View style={styles.resultsTopSection}>
          {/* Badge */}
          <View style={[styles.resultsBadgeCircle, { backgroundColor: `${badge.color}20`, borderColor: `${badge.color}40` }]}>
          ...
          <View style={styles.starsRow}>
            {Array.from({ length: 5 }).map((_, i) => (
              <Text key={i} style={[styles.starIcon, i < stars && { opacity: 1 }]}>⭐</Text>
            ))}
          </View>

          <Text style={styles.resultsHeadline}>
            {stars >= 4 ? "Amazing budgeting! 🎉" : stars >= 2 ? "Nice work! 👏" : "Good effort! 💪"}
          </Text>
      ...
      <Animated.View style={anim(1)}>
        <View style={styles.resultsSummaryCard}>
            ...
            return (
              <View key={cat.id} style={styles.resultsSummaryRow}>
                ...
                <View style={styles.resultsSummaryRight}>
                  <Text style={[styles.resultsSummaryAmount, { color: cat.color }]}>{formatPound(val)}</Text>
          ...
          <View style={styles.resultsSummaryRow}>
            ...
            <Text style={[styles.resultsSummaryAmountBold, { color: remaining >= 0 ? "#10B981" : "#EF4444" }]}>
      ...
      <Animated.View style={anim(2)}>
        <View style={styles.resultsSummaryCard}>
          ...
          <View style={styles.resultsSummaryRow}>
            ...
            <Text style={[styles.resultsSummaryAmount, { color: "#EF4444" }]}>−{formatPound(event.cost)}</Text>
          </View>
          <View style={styles.resultsSummaryRow}>
          ...
          <View style={styles.resultsSummaryRow}>
            ...
            <Text style={[styles.resultsSummaryAmountBold, { color: couldAffordEvent ? "#10B981" : "#EF4444" }]}>
...
export default function BudgetSimulatorScreen() {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Sparkle dots */}
      {[...Array(10)].map((_, i) => (
        <View key={`s-${i}`} style={[styles.star, { left: (i * 79 + 41) % width, top: (i * 53 + 29) % (height * 0.3), width: i % 3 === 0 ? 4 : 2, height: i % 3 === 0 ? 4 : 2, opacity: 0.1 + (i % 4) * 0.05 }]} />
      ))}

      {/* Floating coins */}
      <FloatingCoin delay={0} startX={width * 0.07} startY={height * 0.04} size={18} opacity={0.22} />
      <FloatingCoin delay={400} startX={width * 0.86} startY={height * 0.07} size={14} opacity={0.18} />
          ...
          <TouchableOpacity
            onPress={() => {
              if (step === "job-select") {
                ...
                // Go back one step
                const steps: SimStep[] = ["job-select", "payslip", "budget", "life-event", "results"];
...
const styles = StyleSheet.create({
  ...
  topBarTitle: { fontFamily: "Fredoka_700Bold", fontSize: 20, color: "#FFFFFF", textShadowColor: "rgba(0,0,0,0.2)", textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 4 },
  ...
  stepDot: {
    ...
    borderWidth: 1.5, borderColor: "rgba(255,255,255,0.12)", alignItems: "center", justifyContent: "center",
  },
  ...
  stepLabel: { fontFamily: "Poppins_400Regular", fontSize: 9, color: "rgba(255,255,255,0.3)", position: "absolute", top: 32, width: 50, textAlign: "center", left: -11 },
  stepLine: { width: 20, height: 2, backgroundColor: "rgba(255,255,255,0.1)", marginHorizontal: 4 },

  // ── Step Header ──
  stepHeaderRow: { flexDirection: "row", alignItems: "center", gap: 14, marginBottom: 10, marginTop: 8 },
  stepIconCircle: {
    width: 60, height: 60, borderRadius: 18, backgroundColor: "rgba(255,215,0,0.12)",
    borderWidth: 1, borderColor: "rgba(255,215,0,0.15)", alignItems: "center", justifyContent: "center",
  },
  stepTitle: {
    fontFamily: "Fredoka_700Bold", fontSize: 24, color: "#FFFFFF",
    textShadowColor: "rgba(0,0,0,0.15)", textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3,
  },
  stepSubtitle: { fontFamily: "Poppins_400Regular", fontSize: 13, color: "rgba(255,255,255,0.45)", marginTop: 2 },
  stepDescription: {
    fontFamily: "Poppins_400Regular", fontSize: 14, color: "rgba(255,255,255,0.6)",
  ...
  jobCard: {
    ...
    paddingVertical: 18, paddingHorizontal: 16, gap: 14, marginBottom: 12,
  },
  jobEmojiCircle: { width: 60, height: 60, borderRadius: 18, alignItems: "center", justifyContent: "center" },
  jobCardText: { flex: 1 },
  jobTitle: { fontFamily: "Poppins_700Bold", fontSize: 16, color: "#FFFFFF" },
  jobDescription: { fontFamily: "Poppins_400Regular", fontSize: 12, color: "rgba(255,255,255,0.5)", marginTop: 3, lineHeight: 18 },
  jobSalaryRow: { flexDirection: "row", alignItems: "center", marginTop: 6, gap: 6 },
  jobSalary: { fontFamily: "Poppins_700Bold", fontSize: 14 },
  jobArrow: { width: 36, height: 36, borderRadius: 12, alignItems: "center", justifyContent: "center" },

  // ── Payslip ──
  payslipCard: {
    backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 20, borderWidth: 1,
    borderColor: "rgba(255,255,255,0.08)", overflow: "hidden", marginBottom: 16,
  },
  payslipHeader: { paddingVertical: 14, paddingHorizontal: 18 },
  payslipHeaderLabel: { fontFamily: "Poppins_600SemiBold", fontSize: 10, color: "rgba(255,255,255,0.4)", letterSpacing: 1 },
  payslipHeaderRole: { fontFamily: "Poppins_700Bold", fontSize: 16, color: "#FFFFFF", marginTop: 2 },
  payslipRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 18, paddingVertical: 12 },
  ...
  payslipHint: { fontFamily: "Poppins_400Regular", fontSize: 11, color: "rgba(255,255,255,0.35)", marginTop: 2 },
  ...
  payslipSectionLabel: { fontFamily: "Poppins_600SemiBold", fontSize: 10, color: "rgba(255,255,255,0.35)", letterSpacing: 1, paddingHorizontal: 18, paddingTop: 10 },

  // ── Payslip Bar ──
  payslipBarContainer: { marginBottom: 16 },
  payslipBarTrack: { flexDirection: "row", height: 10, borderRadius: 5, overflow: "hidden", backgroundColor: "rgba(255,255,255,0.06)" },
  payslipBarFill: { height: "100%" },
  payslipBarLabels: { flexDirection: "row", justifyContent: "space-between", marginTop: 6 },
  payslipBarLabelText: { fontFamily: "Poppins_400Regular", fontSize: 11, color: "rgba(255,255,255,0.45)" },

  // ── Info Box ──
  infoBox: {
    backgroundColor: "rgba(79,195,247,0.08)", borderRadius: 14, padding: 16, marginBottom: 16,
    borderWidth: 1, borderColor: "rgba(79,195,247,0.12)",
  },
  infoBoxTitle: { fontFamily: "Poppins_600SemiBold", fontSize: 14, color: "#4FC3F7", marginBottom: 6 },
  infoBoxText: { fontFamily: "Poppins_400Regular", fontSize: 13, color: "rgba(255,255,255,0.6)", lineHeight: 20 },

  // ── CTA Button ──
  ctaButton: {
    borderRadius: 20, paddingVertical: 16, alignItems: "center", justifyContent: "center",
    marginBottom: 8,
    ...Platform.select({
      ios: { shadowColor: "#F5A623", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 12 },
      android: { elevation: 6 },
    }),
  },
  ...
  secondaryButtonText: { fontFamily: "Poppins_600SemiBold", fontSize: 15, color: "rgba(255,255,255,0.7)" },

  // ── Budget Slider ──
  sliderContainer: {
    backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 16, borderWidth: 1,
    borderColor: "rgba(255,255,255,0.06)", paddingVertical: 14, paddingHorizontal: 16, marginBottom: 10,
  },
  ...
  sliderBtn: {
    width: 36, height: 36, borderRadius: 12, borderWidth: 1.5, alignItems: "center", justifyContent: "center",
    backgroundColor: "rgba(255,255,255,0.04)",
  },
  sliderBtnText: { fontFamily: "Poppins_700Bold", fontSize: 20, marginTop: -2 },
  sliderTrack: { flex: 1, height: 8, borderRadius: 4, backgroundColor: "rgba(255,255,255,0.06)", overflow: "hidden", position: "relative" },
  ...
  sliderSuggestedText: { fontFamily: "Poppins_400Regular", fontSize: 10, color: "rgba(255,255,255,0.3)", marginTop: 6 },

  // ── Money Left ──
  moneyLeftCard: {
    backgroundColor: "rgba(255,255,255,0.05)", borderRadius: 18, borderWidth: 1,
    borderColor: "rgba(255,255,255,0.08)", padding: 16, marginBottom: 18,
  },
  moneyLeftTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 },
  moneyLeftLabel: { fontFamily: "Poppins_500Medium", fontSize: 12, color: "rgba(255,255,255,0.45)", marginBottom: 2 },
  moneyLeftAmount: { fontFamily: "Fredoka_700Bold", fontSize: 28 },
  moneyLeftSpent: { fontFamily: "Poppins_600SemiBold", fontSize: 14, color: "rgba(255,255,255,0.6)", textAlign: "right" },
  moneyLeftTrack: { height: 8, borderRadius: 4, backgroundColor: "rgba(255,255,255,0.06)", overflow: "hidden" },
  moneyLeftFill: { height: "100%", borderRadius: 4 },
  overspendWarning: {
    flexDirection: "row", alignItems: "center", gap: 8, marginTop: 12,
    backgroundColor: "rgba(239,68,68,0.1)", borderRadius: 12, paddingVertical: 10, paddingHorizontal: 12,
  ...
  // ── Life Event ──
  lifeEventCard: {
    borderRadius: 20, borderWidth: 1, padding: 24, alignItems: "center", marginBottom: 20, marginTop: 4,
  },
  lifeEventEmoji: { fontSize: 48, marginBottom: 12 },
  lifeEventTitle: { fontFamily: "Fredoka_700Bold", fontSize: 22, color: "#FFFFFF", textAlign: "center", marginBottom: 8 },
  lifeEventDesc: { fontFamily: "Poppins_400Regular", fontSize: 14, color: "rgba(255,255,255,0.6)", textAlign: "center", lineHeight: 22, marginBottom: 14 },
  ...
  impactRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 6 },
  ...
  impactBanner: {
    flexDirection: "row", alignItems: "center", gap: 10, marginTop: 14, borderRadius: 14,
  ...
  // ── Mini Progress Bar ──
  miniProgressTrack: { height: 6, borderRadius: 3, backgroundColor: "rgba(255,255,255,0.08)", overflow: "hidden", width: "100%" },
  ...
  resultsBadgeLabel: { fontFamily: "Poppins_600SemiBold", fontSize: 14, color: "#FFD700", marginBottom: 8 },
  starsRow: { flexDirection: "row", gap: 4, marginBottom: 12 },
  starIcon: { fontSize: 22, opacity: 0.2 },
  resultsHeadline: {
    fontFamily: "Fredoka_700Bold", fontSize: 26, color: "#FFFFFF", textAlign: "center",
    textShadowColor: "rgba(0,0,0,0.15)", textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3,
  },
  resultsSubheadline: { fontFamily: "Poppins_400Regular", fontSize: 13, color: "rgba(255,255,255,0.5)", textAlign: "center", marginTop: 6 },

  resultsSummaryCard: {
    backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 18, borderWidth: 1,
    borderColor: "rgba(255,255,255,0.08)", padding: 18, marginBottom: 14,
  },
  resultsSummaryTitle: { fontFamily: "Poppins_700Bold", fontSize: 15, color: "#FFFFFF", marginBottom: 14 },
  resultsSummaryRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 6 },
  resultsSummaryLeft: { flexDirection: "row", alignItems: "center", gap: 8, flex: 1 },
  resultsSummaryEmoji: { fontSize: 18 },
  resultsSummaryLabel: { fontFamily: "Poppins_400Regular", fontSize: 13, color: "rgba(255,255,255,0.6)" },
  ...
  resultsSummaryPct: { fontFamily: "Poppins_400Regular", fontSize: 11, color: "rgba(255,255,255,0.35)", width: 30, textAlign: "right" },

  resultsTipsCard: {
    backgroundColor: "rgba(168,85,247,0.08)", borderRadius: 18, borderWidth: 1,
    borderColor: "rgba(168,85,247,0.12)", padding: 18, marginBottom: 8,
  },
  resultsTipsTitle: { fontFamily: "Poppins_700Bold", fontSize: 15, color: "#A855F7", marginBottom: 10 },
  resultsTipRow: { flexDirection: "row", gap: 8, marginBottom: 6 },
  resultsTipBullet: { fontFamily: "Poppins_400Regular", fontSize: 14, color: "rgba(255,255,255,0.4)", lineHeight: 20 },
  resultsTipText: { fontFamily: "Poppins_400Regular", fontSize: 13, color: "rgba(255,255,255,0.6)", flex: 1, lineHeight: 20 },
});