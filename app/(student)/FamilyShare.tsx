  ...
  return (
    <Animated.View style={{ position: "absolute", left: startX, top: startY, opacity: Animated.multiply(fadeIn, opacity), transform: [{ translateY }], zIndex: 0 }}>
      <Svg width={size} height={size} viewBox="0 0 40 40">
        <Defs><SvgLinearGradient id={`fsCoin-${delay}`} x1="0" y1="0" x2="0" y2="1"><Stop offset="0" stopColor="#FFD700" /><Stop offset="1" stopColor="#F5A623" /></SvgLinearGradient></Defs>
        <Circle cx="20" cy="20" r="18" fill={`url(#fsCoin-${delay})`} /><Circle cx="20" cy="20" r="14" fill="none" stroke="#E8960C" strokeWidth="1.5" />
...
const ShieldIcon = ({ size = 48 }: { size?: number }) => (
  <Svg width={size} height={size} viewBox="0 0 48 48">
    <Path d="M24 4 L40 12 L40 24 Q40 36 24 44 Q8 36 8 24 L8 12 Z" fill="none" stroke="#10B981" strokeWidth="2.5" />
    <Path d="M24 8 L36 14 L36 24 Q36 34 24 40 Q12 34 12 24 L12 14 Z" fill="#10B981" opacity={0.12} />
    <Path d="M17 24 L22 29 L31 19" fill="none" stroke="#10B981" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
  </Svg>
);
...
const LinkIcon = ({ size = 40 }: { size?: number }) => (
  <Svg width={size} height={size} viewBox="0 0 40 40">
    ...
    <Path d="M22 14 L26 10 Q30 6 34 10 L30 14 Q34 18 30 22 L26 26" fill="none" stroke="#A855F7" strokeWidth="2.5" strokeLinecap="round" />
    <Path d="M18 26 L14 30 Q10 34 6 30 L10 26 Q6 22 10 18 L14 14" fill="none" stroke="#A855F7" strokeWidth="2.5" strokeLinecap="round" />
  </Svg>
);

const QRPlaceholder = () => (
  <Svg width={140} height={140} viewBox="0 0 140 140">
    <Rect x="4" y="4" width="132" height="132" rx="12" fill="rgba(255,255,255,0.06)" stroke="rgba(255,255,255,0.1)" strokeWidth="2" />
    ...
    {[56, 68, 80, 92, 104].map((x) =>
      [56, 68, 80, 92, 104].map((y) => {
        ...
        return show ? <Rect key={`${x}-${y}`} x={x} y={y} width="8" height="8" rx="1" fill="rgba(255,255,255,0.5)" /> : null;
      })
    )}
    {[56, 68, 80].map((x) =>
      [16, 28, 40].map((y) => {
        const show = (x * y) % 7 < 4;
        return show ? <Rect key={`t-${x}-${y}`} x={x} y={y} width="8" height="8" rx="1" fill="rgba(255,255,255,0.4)" /> : null;
      })
    )}
    {[16, 28, 40].map((x) =>
      [56, 68, 80].map((y) => {
        const show = (x + y) % 5 < 3;
        return show ? <Rect key={`l-${x}-${y}`} x={x} y={y} width="8" height="8" rx="1" fill="rgba(255,255,255,0.4)" /> : null;
      })
    )}
  </Svg>
);
    ...
    <>
      ...
      <Animated.View style={anim(0)}>
        <View style={styles.stepHeaderRow}>
          <View style={[styles.stepIconCircle, { backgroundColor: "rgba(16,185,129,0.12)", borderColor: "rgba(16,185,129,0.18)" }]}>
      ...
      <Animated.View style={anim(1)}>
        <Text style={styles.stepDescription}>
          You can share a simple, positive summary of your learning progress with a parent or guardian. It's completely optional and you're always in control.
        </Text>
      </Animated.View>
                ...
                <View style={styles.activeLinkTop}>
                  ...
                  <View style={styles.activeLinkMeta}>
                    ...
                    {link.feedbackReceived && (
                      <View style={[styles.viewedPill, { backgroundColor: "rgba(16,185,129,0.15)" }]}>
                        <Text style={[styles.viewedPillText, { color: "#10B981" }]}>💬 Feedback</Text>
                      </View>
                    )}
                  </View>
                </View>
      ...
      <Animated.View style={anim(hasActiveLinks ? 6 : 5)}>
        <TouchableOpacity activeOpacity={0.85} onPress={onContinue} style={{ marginTop: 8 }}>
          <LinearGradient colors={["#10B981", "#059669"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.ctaButton}>
...
const ConsentStep = ({
  ...
  return (
    <>
      <Animated.View style={anim(0)}>
        <View style={styles.stepHeaderRow}>
          <View style={[styles.stepIconCircle, { backgroundColor: "rgba(168,85,247,0.12)", borderColor: "rgba(168,85,247,0.18)" }]}>
            <ShieldIcon size={36} />
          </View>
          <View style={{ flex: 1 }}>
      ...
      <Animated.View style={anim(1)}>
        <Text style={styles.stepDescription}>
          Before creating a share link, please review and agree to each item below. This is your data and you're in charge.
        </Text>
      </Animated.View>
      ...
      {CONSENT_ITEMS.map((item, i) => (
        <Animated.View key={item.id} style={anim(i + 2)}>
          <TouchableOpacity
            ...
            style={[
              styles.consentItem,
              consented[item.id] && styles.consentItemActive,
            ]}
          >
      ...
      <Animated.View style={anim(6)}>
        <View style={styles.importantNote}>
          ...
          <Text style={styles.importantNoteText}>
            You can revoke the link at any time from this page. The link expires automatically after 7 days.
          </Text>
        </View>
      </Animated.View>
      ...
      <Animated.View style={anim(6)}>
        ...
        >
          <LinearGradient
            colors={allConsented ? ["#10B981", "#059669"] : ["rgba(16,185,129,0.25)", "rgba(5,150,105,0.15)"]}
  ...
  return (
    <>
      ...
      {confettiAnims.map((ca, i) => {
        ...
        const colors = ["#10B981", "#FFD700", "#FF2E91", "#4FC3F7", "#A855F7", "#F59E0B", "#10B981", "#FFD700"];
        return (
          <Animated.View key={`c-${i}`} pointerEvents="none"
            style={{ position: "absolute", left: width / 2 - 4, top: 200, width: 8, height: 8, borderRadius: i % 2 === 0 ? 4 : 1, backgroundColor: colors[i], opacity, transform: [{ translateX }, { translateY }], zIndex: 20 }} />
        );
      })}

      {/* Success header */}
      <Animated.View style={[anim(0), { alignItems: "center", transform: [{ scale: burstScale }], opacity: burstOpacity }]}>
      ...
      <Animated.View style={anim(4)}>
        <View style={styles.reminderCard}>
          ...
          <Text style={styles.reminderText}>
            This link will expire in <Text style={{ color: "#FFD700", fontFamily: "Poppins_700Bold" }}>7 days</Text>. You can revoke it at any time from the Share with Family page. Only the summary shown in the preview is visible — no scores, mistakes, or personal data.
          </Text>
        </View>
      </Animated.View>
...
export default function FamilyShareScreen() {
  ...
  return (
    <View style={styles.container}>
      ...
      {[...Array(10)].map((_, i) => (
        <View key={`s-${i}`} style={[styles.star, { left: (i * 79 + 43) % width, top: (i * 61 + 21) % (height * 0.3), width: i % 3 === 0 ? 4 : 2, height: i % 3 === 0 ? 4 : 2, opacity: 0.1 + (i % 4) * 0.05 }]} />
      ))}

      <FloatingCoin delay={100} startX={width * 0.07} startY={height * 0.04} size={18} opacity={0.22} />
      <FloatingCoin delay={400} startX={width * 0.85} startY={height * 0.08} size={14} opacity={0.18} />
            ...
            <View key={label} style={styles.stepDotRow}>
              <View style={[
                ...
                {i < stepIndex ? (
                  ...
                  <Text style={[styles.stepDotNumber, i === stepIndex && { color: "#1A1B4B" }]}>{i + 1}</Text>
                )}
              </View>
              <Text style={[styles.stepDotLabel, (i === stepIndex || i < stepIndex) && { color: "rgba(255,255,255,0.6)" }]}>{label}</Text>
              {i < 2 && <View style={[styles.stepLine, i < stepIndex && { backgroundColor: "#10B981" }]} />}
            </View>
          ))}
        </View>
...
const styles = StyleSheet.create({
  ...
  scrollContent: { paddingTop: Platform.OS === "android" ? (StatusBar.currentHeight ?? 0) + 12 : 56, paddingHorizontal: 20 },

  // ── Top Bar ──
  topBar: { flexDirection: "row", alignItems: "center", marginBottom: 16, gap: 12 },
  backButton: { width: 40, height: 40, borderRadius: 14, backgroundColor: "rgba(255,255,255,0.08)", alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: "rgba(255,255,255,0.06)" },
  ...
  stepDot: { width: 30, height: 30, borderRadius: 15, backgroundColor: "rgba(255,255,255,0.08)", borderWidth: 1.5, borderColor: "rgba(255,255,255,0.12)", alignItems: "center", justifyContent: "center" },
  ...
  stepDotNumber: { fontFamily: "Poppins_600SemiBold", fontSize: 12, color: "rgba(255,255,255,0.35)" },
  stepDotLabel: { fontFamily: "Poppins_400Regular", fontSize: 10, color: "rgba(255,255,255,0.25)", position: "absolute", top: 34, width: 60, textAlign: "center", left: -15 },
  ...
  stepIconCircle: { width: 60, height: 60, borderRadius: 18, alignItems: "center", justifyContent: "center", borderWidth: 1 },
  stepTitle: { fontFamily: "Fredoka_700Bold", fontSize: 24, color: "#FFFFFF", textShadowColor: "rgba(0,0,0,0.15)", textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3 },
  stepSubtitle: { fontFamily: "Poppins_400Regular", fontSize: 13, color: "rgba(255,255,255,0.45)", marginTop: 2 },
  stepDescription: { fontFamily: "Poppins_400Regular", fontSize: 14, color: "rgba(255,255,255,0.6)", lineHeight: 22, marginBottom: 20 },

  // ── Section Title ──
  sectionTitle: { fontFamily: "Fredoka_700Bold", fontSize: 18, color: "#FFFFFF", marginBottom: 12, textShadowColor: "rgba(0,0,0,0.15)", textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3 },

  // ── Preview Card ──
  previewCard: { backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 20, borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", padding: 20, marginBottom: 16 },
  previewHeadline: { fontFamily: "Fredoka_700Bold", fontSize: 20, color: "#FFD700", textAlign: "center", marginBottom: 18 },
  previewSection: { marginBottom: 16 },
  previewLabel: { fontFamily: "Poppins_600SemiBold", fontSize: 10, color: "rgba(255,255,255,0.3)", letterSpacing: 1, marginBottom: 8 },
  previewHighlightRow: { flexDirection: "row", gap: 8, marginBottom: 8 },
  previewBullet: { fontSize: 14 },
  previewHighlightText: { fontFamily: "Poppins_400Regular", fontSize: 13, color: "rgba(255,255,255,0.65)", flex: 1, lineHeight: 20 },
  previewBadgeRow: { flexDirection: "row", flexWrap: "wrap", gap: 10 },
  previewBadge: { alignItems: "center", gap: 4, backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 12, paddingVertical: 10, paddingHorizontal: 12, borderWidth: 1, borderColor: "rgba(255,255,255,0.06)" },
  previewBadgeEmoji: { fontSize: 22 },
  previewBadgeName: { fontFamily: "Poppins_500Medium", fontSize: 10, color: "rgba(255,255,255,0.5)" },
  previewStarterRow: { flexDirection: "row", gap: 8, marginBottom: 8 },
  previewStarterEmoji: { fontSize: 14 },
  previewStarterText: { fontFamily: "Poppins_400Regular", fontSize: 13, color: "rgba(255,255,255,0.6)", flex: 1, lineHeight: 20, fontStyle: "italic" },

  // ── Not Shared Card ──
  notSharedCard: { flexDirection: "row", gap: 14, backgroundColor: "rgba(16,185,129,0.06)", borderRadius: 18, borderWidth: 1, borderColor: "rgba(16,185,129,0.12)", padding: 18, marginBottom: 16 },
  notSharedTitle: { fontFamily: "Poppins_700Bold", fontSize: 14, color: "#10B981", marginBottom: 8 },
  ...
  // ── Controls Card ──
  controlsCard: { backgroundColor: "rgba(168,85,247,0.06)", borderRadius: 18, borderWidth: 1, borderColor: "rgba(168,85,247,0.12)", padding: 18, marginBottom: 16 },
  controlsTitle: { fontFamily: "Poppins_700Bold", fontSize: 14, color: "#A855F7", marginBottom: 12 },
  controlItem: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 8 },
  controlEmoji: { fontSize: 16 },
  controlText: { fontFamily: "Poppins_400Regular", fontSize: 13, color: "rgba(255,255,255,0.6)", flex: 1 },

  // ── Active Links ──
  activeLinkCard: { backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 16, borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", padding: 16, marginBottom: 10 },
  activeLinkTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 },
  activeLinkCodeWrap: {},
  activeLinkCodeLabel: { fontFamily: "Poppins_500Medium", fontSize: 10, color: "rgba(255,255,255,0.35)", letterSpacing: 0.5 },
  activeLinkCode: { fontFamily: "Poppins_700Bold", fontSize: 18, color: "#FFD700", marginTop: 2, letterSpacing: 1.5 },
  activeLinkMeta: { alignItems: "flex-end", gap: 4 },
  activeLinkMetaText: { fontFamily: "Poppins_400Regular", fontSize: 11, color: "rgba(255,255,255,0.35)" },
  viewedPill: { backgroundColor: "rgba(79,195,247,0.15)", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8 },
  viewedPillText: { fontFamily: "Poppins_500Medium", fontSize: 10, color: "#4FC3F7" },
  revokeButton: { backgroundColor: "rgba(239,68,68,0.1)", borderRadius: 12, paddingVertical: 10, alignItems: "center", borderWidth: 1, borderColor: "rgba(239,68,68,0.15)" },
  revokeButtonText: { fontFamily: "Poppins_600SemiBold", fontSize: 13, color: "#EF4444" },

  // ── CTA Button ──
  ctaButton: {
    borderRadius: 20, paddingVertical: 16, alignItems: "center", justifyContent: "center", marginBottom: 8,
    ...Platform.select({ ios: { shadowColor: "#10B981", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 12 }, android: { elevation: 6 } }),
  },
  ctaText: { fontFamily: "Poppins_700Bold", fontSize: 16, color: "#FFFFFF", letterSpacing: 0.3 },

  // ── Secondary Button ──
  secondaryButton: { borderRadius: 20, paddingVertical: 14, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.1)", marginTop: 8 },
  secondaryButtonText: { fontFamily: "Poppins_600SemiBold", fontSize: 14, color: "rgba(255,255,255,0.6)" },

  // ── Consent Items ──
  consentItem: { flexDirection: "row", gap: 14, backgroundColor: "rgba(255,255,255,0.03)", borderRadius: 16, borderWidth: 1.5, borderColor: "rgba(255,255,255,0.06)", padding: 16, marginBottom: 10 },
  consentItemActive: { borderColor: "rgba(16,185,129,0.35)", backgroundColor: "rgba(16,185,129,0.06)" },
  consentCheckbox: { marginTop: 2 },
  consentUnchecked: { width: 24, height: 24, borderRadius: 8, borderWidth: 2, borderColor: "rgba(255,255,255,0.2)", backgroundColor: "rgba(255,255,255,0.04)" },
  consentChecked: { width: 24, height: 24, borderRadius: 8, backgroundColor: "#10B981", alignItems: "center", justifyContent: "center" },
  consentCheckmark: { fontFamily: "Poppins_700Bold", fontSize: 14, color: "#FFFFFF", marginTop: -1 },
  ...
  consentDetail: { fontFamily: "Poppins_400Regular", fontSize: 12, color: "rgba(255,255,255,0.45)", lineHeight: 18 },

  // ── Important Note ──
  importantNote: { flexDirection: "row", gap: 10, backgroundColor: "rgba(79,195,247,0.06)", borderRadius: 14, padding: 14, borderWidth: 1, borderColor: "rgba(79,195,247,0.1)", marginBottom: 16, marginTop: 8 },
  importantNoteEmoji: { fontSize: 16 },
  importantNoteText: { fontFamily: "Poppins_400Regular", fontSize: 12, color: "rgba(255,255,255,0.5)", flex: 1, lineHeight: 18 },

  // ── Generate Step ──
  successIconCircle: { width: 90, height: 90, borderRadius: 28, backgroundColor: "rgba(16,185,129,0.15)", borderWidth: 2, borderColor: "rgba(16,185,129,0.25)", alignItems: "center", justifyContent: "center", marginBottom: 14, marginTop: 8 },
  successTitle: { fontFamily: "Fredoka_700Bold", fontSize: 28, color: "#FFFFFF", textAlign: "center", marginBottom: 6, textShadowColor: "rgba(0,0,0,0.15)", textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 3 },
  successSubtitle: { fontFamily: "Poppins_400Regular", fontSize: 14, color: "rgba(255,255,255,0.5)", textAlign: "center", marginBottom: 24 },

  // ── Code Card ──
  codeCard: { alignItems: "center", backgroundColor: "rgba(255,215,0,0.08)", borderRadius: 20, borderWidth: 1, borderColor: "rgba(255,215,0,0.15)", padding: 24, marginBottom: 16 },
  codeCardLabel: { fontFamily: "Poppins_600SemiBold", fontSize: 10, color: "rgba(255,255,255,0.35)", letterSpacing: 1.5, marginBottom: 8 },
  codeCardValue: { fontFamily: "Fredoka_700Bold", fontSize: 36, color: "#FFD700", letterSpacing: 3, marginBottom: 16, textShadowColor: "rgba(255,215,0,0.3)", textShadowOffset: { width: 0, height: 2 }, textShadowRadius: 8 },
  copyButton: { backgroundColor: "rgba(255,215,0,0.15)", paddingHorizontal: 20, paddingVertical: 10, borderRadius: 14, borderWidth: 1, borderColor: "rgba(255,215,0,0.2)" },
  copyButtonText: { fontFamily: "Poppins_600SemiBold", fontSize: 14, color: "#FFD700" },

  // ── QR Card ──
  qrCard: { alignItems: "center", backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 20, borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", padding: 24, marginBottom: 16 },
  qrCardLabel: { fontFamily: "Poppins_600SemiBold", fontSize: 10, color: "rgba(255,255,255,0.3)", letterSpacing: 1.5, marginBottom: 16 },
  qrPlaceholder: { marginBottom: 12 },
  qrHint: { fontFamily: "Poppins_400Regular", fontSize: 12, color: "rgba(255,255,255,0.35)", textAlign: "center" },

  // ── How To Card ──
  howToCard: { backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 18, borderWidth: 1, borderColor: "rgba(255,255,255,0.06)", padding: 18, marginBottom: 16 },
  ...
  // ── Reminder Card ──
  reminderCard: { backgroundColor: "rgba(168,85,247,0.06)", borderRadius: 18, borderWidth: 1, borderColor: "rgba(168,85,247,0.12)", padding: 18, marginBottom: 8 },
  reminderTitle: { fontFamily: "Poppins_700Bold", fontSize: 14, color: "#A855F7", marginBottom: 8 },
  reminderText: { fontFamily: "Poppins_400Regular", fontSize: 13, color: "rgba(255,255,255,0.55)", lineHeight: 20 },
});